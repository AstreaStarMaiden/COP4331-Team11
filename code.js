		...
		xhr.onreadystatechange = function() 
		{
			if (this.readyState == 4 && this.status == 200) 
			{
				...
				try
				{
					...
					{
						// Assuming each result has a 'name' property (adjust if needed)
					...
					// Use a specific ID instead of relying on <p> tag index
					document.getElementById("contactListDisplay").innerHTML = contactList;
				}
				catch(parseError)
				{
					console.error("Error parsing JSON response: ", parseError);
					document.getElementById("contactSearchResult").innerHTML = "Error parsing contact information.";
				}
			}
		};
		xhr.send(jsonPayload);
